...
$routes->group('api/auth', function ($routes) {
    ...
    $routes->post('refresh', 'AuthController::refresh', ['filter' => 'jwt']); // Menambahkan filter jwt
});

// User routes - Bug #2: Missing API prefix consistency
// $routes->group('users', ['filter' => 'jwt'], function ($routes) {
$routes->group('api/users', ['filter' => 'jwt'], function ($routes) { // Perbaikan prefix konsisten: pakai "api/users"
...
$routes->group('api/tasks', ['filter' => 'jwt'], function ($routes) { // Perbaikan filter: auth → jwt
